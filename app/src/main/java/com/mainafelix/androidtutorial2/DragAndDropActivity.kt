...
class DragAndDropActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val view = findViewById<View>(R.id.dragView)
        val llTop  = findViewById<LinearLayout>(R.id.llTop)
        val llBottom  = findViewById<LinearLayout>(R.id.llBottom)
        llTop.setOnDragListener(dragLitener)
        llBottom.setOnDragListener(dragLitener)
        view.setOnClickListener{
            val  clipText ="this is our clip text"
    ...
    val dragLitener  = View.OnDragListener{view , event ->
       when (event.action){
